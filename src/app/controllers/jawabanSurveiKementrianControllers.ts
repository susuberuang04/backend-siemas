...
import { UsulanSmart } from "../../database/models/usulansmart";
import { Opd } from "../../database/models/opd";
import { User } from "../../database/models/user";
const { JawabanSurveiKementrian } = models;

// Konfigurasi Penyimpanan File
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, "../../public/survei/kementrian/"));
  },
...
const JawabanSurveiKementrianController = {
  createSurveiKementrian: async (req: Request, res: Response) => {
    ...
    try {
      upload(req, res, async (err) => {
        ...
        const newEntry = await JawabanSurveiKementrian.create(
          {
            user_id,
  ...
  getAllSudahMenguploadUniqueSuperadmin: async (
    ...
    try {
      const { count, rows: jawabanSurveiKementrian }: any =
        await JawabanSurveiKementrian.findAndCountAll({
          limit,
          offset,
  ...
  getAllByUserId: async (req: Request, res: Response) => {
    ...
    try {
      const jawabanSurveiKementrian: any =
        await JawabanSurveiKementrian.findAll({
          where: {
            user_id: user_id,
          },
        });
...
export default JawabanSurveiKementrianController;
